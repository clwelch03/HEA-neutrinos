...
energy_shift(neutron_spin, proton_spin, magnetic_field) = NUCLEON_MASS_SPLITTING +
...
function opacity(channel, baryon_density, proton_fraction, neutrino_energy, neutrino_angle, magnetic_field, temperature)
    ...
    if channel == 'p'
        if regime == strong_field
            ...
            for neutron_spin in (-0.5, 0.5), proton_spin in (-0.5, 0.5)
                ...
                sign_sum += reduced_matrix_element_term * exponential_term * (bracketed_term_1 - bracketed_term_2 * bracketed_term_3 * bracketed_term_4)
            end
    ...
    elseif channel == 'n'
        if regime == strong_field
            ...
            sign_sum = 0
            for neutron_spin in (-0.5, 0.5), proton_spin in (-0.5, 0.5)
                E_0_plus_prime = neutrino_energy + energy_shift(neutron_spin, proton_spin, magnetic_field) + 2 * proton_spin * mu_times_B
                recurring_exponential = 1 - exp(-magnetic_field/M_times_T)
                
                reduced_matrix_element_term = reduced_matrix_element(neutron_spin, proton_spin, neutrino_angle; is_strong_field=true)
                fermi_dirac_term = 1 - fermi_dirac(E_0_plus_prime, electron_chemical_potential, temperature)
                exponential_term = exp(-NEUTRON_GYROMAGNETIC_RATIO * neutron_spin * magnetic_field / (2 * M_times_T))

                bracketed_term_1 = proton_density * exp(-(PROTON_GYROMAGNETIC_RATIO-2) * proton_spin * magnetic_field / (2 * M_times_T))
                bracketed_term_2 = (pi / M_times_T)^(3/2) * tanh(magnetic_field / (2 * M_times_T))
                ...
                bracketed_term_4 = exp(-neutrino_momentum_perp^2 / (2 * magnetic_field + 2 * M_times_T) * 
                                        ...
                                        neutrino_momentum_z^2 / (4 * M_times_T))
                sign_sum += reduced_matrix_element_term * fermi_dirac_term * exponential_term *
...
println(opacity('n', (197.3^3 * 0.16 * 0.001), 0.25, 10, pi/2, ELEM_CHARGE*GAUSS_TO_MEV2*1e17, 1))
